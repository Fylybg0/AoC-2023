    ...
    for mini in minimals:
        if mini[0] != end:
            for axe in axes:
                ...
                if 0 <= q < len(f[0]) and 0 <= p < len(f) and ((axe == mini[2] and mini[1] < 10) or (axe != mini[2] and (10 > mini[1] >= 4 or mini[1] == 0))) and minimals[mini] + f[p][q] < low and (axe[0]*-1, axe[1]*-1) != mini[2]:
                    new_key = ((q, p), mini[1] + 1 if mini[2] == axe else 1, axe)
                    if (q, p) == end and low > minimals[mini] + f[p][q] and mini[1] >= 3:
