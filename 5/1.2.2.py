...
def overlap(x, y):      
    if (x[1] >= y[1] and x[0] <= y[0]):
        ...
        #print(x, y, (y[0], x[1]), [(x[0], y[0] - 1)] if x[0] != y[0] else [])
        return (y[0], x[1]), [(x[0], y[0] - 1)] if x[0] != y[0] else []
    elif (x[0] >= y[0] and x[0] <= y[1] <= x[1]):
        #print(x, y, (x[0], y[1]), [(y[1] + 1, x[1])]  if x[1] != y[1] else [])
...
for i in range(len(a)):
    ...
    while current != 'location':
        ...
        print(ranges)
    print(min(ranges, key=lambda x: x[0]), best)
